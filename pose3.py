...
def save_keypoints(frame3, keypoints3, confidence_threshold):
    ...
    for kp in shaped:
        ...
        if kp_conf > confidence_threshold:
            ...
            continue
    if count == 7:
        left = triangle(temp[0][0], temp[1][0], temp[5][0], temp[0][1], temp[1][1], temp[5][1])
        # triangle(x_1, x_2, x_3, y_1, y_2, y_3)
        right = triangle(temp[6][0], temp[2][0], temp[0][0], temp[6][1], temp[2][1], temp[0][1])
...
while cap.isOpened():
    ...
    draw_keypoints(frame, keypoints_with_scores[:][:][:7], 0.4)
    Rate_Le_Ri = save_keypoints(frame, keypoints_with_scores[0][0][:7], 0.4)
    ...
    if cv2.waitKey(20) & 0XFF == ord('q'):
        ...
        new_rate = pd.DataFrame(Rates, columns=['ratio', 'left area', 'right area'])
        print(new_rate)
